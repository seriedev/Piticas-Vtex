...
class Helpers {
  ...
  _formatReal(int) {
    ...
    //com R$
    let f = atual.toLocaleString('pt-br',{style: 'currency', currency: 'BRL'});
    //sem R$
    let f2 = atual.toLocaleString('pt-br', {minimumFractionDigits: 2});

    return f2;
  }

  _formatCurrencyDecimal(numero) {
    const str = numero.toFixed(0);
    const resStr = str.substring(0, str.length - 2) + '.' + str.substring(str.length - 2);
    const resposta = parseFloat(resStr);
